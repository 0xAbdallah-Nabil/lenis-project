...
export default function FileExplorer({ onOpenFile, activeFileId }: FileExplorerProps) {
    ...
    const fileClasses = (id: string) =>
        `flex items-center gap-1 w-full px-3 py-1.5 transition-colors ${activeFileId === id
            ? "text-slate-900 dark:text-white bg-slate-200/60 dark:bg-slate-800/60"
    ...
    return (
        <aside className="w-full md:w-64 shrink-0 self-start md:self-stretch h-fit md:h-full bg-white/90 dark:bg-[#0a1628d6] border-r border-slate-200 dark:border-slate-800 font-mono text-sm text-slate-700 dark:text-slate-300 transition-colors">
            {/* root folder */}
            <button
                onClick={() => toggleFolder("root")}
                className="flex items-center gap-1 w-full px-3 py-2 text-slate-800 dark:text-slate-200 font-medium hover:bg-slate-200/40 dark:hover:bg-slate-800/40"
            >
            ...
            {openFolders.root && (
                <div className="pl-4">
                    ...
                    <button
                        ...
                        className="flex items-center gap-1 w-full px-3 py-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:bg-slate-200/40 dark:hover:bg-slate-800/40"
                    >
            ...
            {/* contacts */}
            <div className=" md:block hidden mt-4 px-3 py-2 text-slate-800 dark:text-slate-200 font-medium border-t border-slate-200 dark:border-slate-800">
                contacts
            </div>
            <div className="pl-4 pb-4 md:flex md:flex-col md:items-start md:gap-2 hidden">
                <a
                    href="mailto:abdallahnabil2003@gmail.com"
                    className="flex items-center justify-start  px-3 py-1.5 text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200"
                >

                    abdallahnabil2003@gmail.com
                </a>
                <div className="flex items-center gap-2 px-3 py-1.5 text-slate-500 dark:text-slate-400">
                    <a className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:scale-120" href="https://github.com/0xAbdallah-Nabil" target="_blank" rel="noopener noreferrer">
                        <FaGithub size={18} />
                    </a>
                    <a className="text-slate-500 dark:text-slate-400 hover:text-slate-800 dark:hover:text-slate-200 hover:scale-120" href="https://www.linkedin.com/in/0xabdallah-nabil/" target="_blank" rel="noopener noreferrer">
                        <FaLinkedin size={18} />
                    </a>
